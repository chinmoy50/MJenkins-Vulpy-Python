        ...
        stage('Perform SAST Scan') {
            steps {
                script {
                    def response = sh(script: """
                        #!/bin/bash
                        curl -v -X POST                         -H "Client-ID: ${CLIENT_ID}"                         -H "Client-Secret: ${CLIENT_SECRET}"                         -F "projectZipFile=@project.zip"                         -F "applicationId=${APPLICATION_ID}"                         -F "scanName=Vulpy-Pyhton-SAST Scan"                         -F "language=python"                         "${SAST_API_URL}"
                    """, returnStdout: true).trim()
